...
\begin{frame}{Phase 2: Learn. POD Basis from Snapshots}
  ...
  \medskip
  Take the SVD $X = U \Sigma W^{\top}$. Keep the top $r$ left singular vectors:
  \[
    V = U[:, 1{:}r] \in \mathbb{R}^{N \times r}.
  \]

  \medskip
  $V$ is the optimal rank-$r$ orthonormal basis for the snapshot data in the $L^2$ sense. The reduced state is $a \in \mathbb{R}^r$, with
  \[
    z \;\approx\; V a.
  \]
...
\begin{frame}{Benchmarks}
  \begin{center}
    ...
    \begin{tabular}{lcccc}
      \toprule
      System                & $N$ orig & $N$ lifted & Aux added & ROM error vs full \\
      \midrule
      Cubic decay $\dot{x}=-x^3$ & 1   & 2   & 1  & $\sim 10^{-10}$ \\
      Van der Pol               & 2   & 4   & 2  & $\sim 10^{-9}$  \\
      Lorenz                    & 3   & 3   & 0  & $\sim 10^{-10}$ \\
      Allen--Cahn (1D)          & $n$ & $2n$ & $n$ & $\sim 10^{-7}$  \\
      \bottomrule
    \end{tabular}
  \end{center}

  \medskip
  Lift correctness verified by integrating both the lifted and unlifted systems with \texttt{Tsit5} at \texttt{abstol = reltol = 1e-10} and comparing the original-state slice. ROM error reflects the gap between the full lifted simulation and the reduced one at the same modes.

  \medskip
  Test suite: 596 tests, runs in ~3 s after package load. CI on every push.
\end{frame}

\begin{frame}{Tensor vs Dense Quadratic Evaluation}
  Dense $\widehat{H}\, (a \otimes a)$ vs \texttt{QuadraticTensor} contraction, on Allen--Cahn-like sparsity:
  ...
  \begin{center}
    ...
    \begin{tabular}{rcccc}
      \toprule
      $r$ & dense (ms) & tensor (ms) & speedup & memory ratio \\
      \midrule
      4   & X.X & X.X & X.Xx & X.Xx \\
      8   & X.X & X.X & X.Xx & X.Xx \\
      16  & X.X & X.X & X.Xx & X.Xx \\
      \bottomrule
    \end{tabular}
  \end{center}

  \medskip
  \textcolor{gray}{(Live numbers from \texttt{benchmarks/} populated the day of the talk.)}

  \medskip
  Crossover behavior: dense wins when $\widehat{H}$ is small enough to fit in cache; tensor wins past that. The ROM application path uses the cheapest of the three.
\end{frame}
